...
EXPORT xbee_err xbee_threadValidate(struct xbee *xbee,
...
void threadFunc_cleanup(void *thread) {
    struct xbee_threadInfo *xbee_thread = (struct xbee_threadInfo *)thread;
    xbee_thread->active = 0;
}
void *threadFunc(struct xbee_threadInfo *thread) {
    ...
    if (thread->restartDelay < 0) {
        /* a restartDelay of < 0 indicates that the thread should not restart
         * (by default, the thread can request that it is restarted, in which
         * case -(thread->restartDelay) is used as the delay) */
        restart = 0;
    ...
    do {
        xbee_log(15, "starting thread %p, function %s()...", thread,
                 thread->funcName);
        ...
        if (ret != XBEE_ENONE) {
            xbee_log(1, "thread %p, function %s() returned %d...", thread,
                     thread->funcName, ret);
        } else {
            xbee_log(10, "thread %p, function %s() returned without error...",
                     thread, thread->funcName, ret);
        }
        if (!restart || !thread->run) break;
        if (xbee->die) {
            xbee_log(20,
                     "NOT restarting thread %p, function %s() - libxbee "
                     "instance has been marked for death...",
                     thread, thread->funcName);
        } else if (thread->restartDelay != 0) {
            xbee_log(20, "restarting thread %p, function %s() in %d us...",
                     thread, thread->funcName, thread->restartDelay);
            usleep(thread->restartDelay);
        } else {
            xbee_log(20,
                     "restarting thread %p, function %s() with zero delay...",
                     thread, thread->funcName);
        }
    ...
    if (restart != -1)
        xbee_log(15, "thread %p, function %s() has now ended...", thread,
                 thread->funcName);
...
xbee_err _xbee_threadStart(
    struct xbee *xbee, struct xbee_threadInfo **retThread, int restartDelay,
    int detach, const char *funcName,
    xbee_err (*func)(struct xbee *xbee, int *restart, void *arg), void *arg) {
    struct xbee_threadInfo *thread;
    ...
    thread->active = 1;
    if ((xsys_thread_create(&thread->tid, (void *(*)(void *))threadFunc,
                            thread)) != 0) {
...
xbee_err xbee_threadJoin(struct xbee *xbee, struct xbee_threadInfo *thread,
                         xbee_err *retVal) {
    if (!xbee || !thread) return XBEE_EMISSINGPARAM;
#ifndef XBEE_DISABLE_STRICT_OBJECTS
    if (xbee_threadValidate(xbee, thread) != XBEE_ENONE) return XBEE_EINVAL;
...
xbee_err xbee_threadKillJoin(struct xbee *xbee, struct xbee_threadInfo *thread,
                             xbee_err *retVal) {
    xbee_err ret;
...
xbee_err xbee_threadStopRelease(struct xbee *xbee,
                                struct xbee_threadInfo *thread) {
    xbee_err ret;
...
xbee_err xbee_threadKillRelease(struct xbee *xbee,
                                struct xbee_threadInfo *thread) {
    xbee_err ret;
...
xbee_err xbee_threadDestroyMine(struct xbee *xbee) {
    ...
    ret = XBEE_ENONE;
    for (thread = NULL;
         xbee_ll_get_next(threadList, thread, (void **)&thread) == XBEE_ENONE &&
         thread;) {
        if (thread->xbee != xbee) {
            pThread = thread;
            continue;
